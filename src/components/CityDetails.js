...
export class CityDetails extends Component {
    render() {
        const { cod } = this.props.weatherInfo;
        if (cod == "200") {
            const { name, dt } = this.props.weatherInfo;
            const { icon, description } = this.props.weatherInfo.weather[0];
            const { temp, humidity } = this.props.weatherInfo.main;
            const { speed } = this.props.weatherInfo.wind;
            // const { country } = this.props.weatherInfot.sys;
